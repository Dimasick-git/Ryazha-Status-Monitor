...
# Требования:
- Ryazhahand Overlay или Tesla Menu (версия >=1.2.3)

Как настроить всё: [ЗДЕСЬ](https://gist.github.com/masagrator/65fcbd5ad09243399268d145aaab899b)

## Совместимость с оригинальным Status Monitor Overlay
- Поддержан путь `sdmc:/switch/.overlays/Status-Monitor-Overlay.ovl` как резервный вариант для пользователей, которые обновляются с оригинального оверлея и ещё не переименовали файл.
- Сохранена совместимость с legacy-аргументами микрорежима `--microOverlay` и `--microOverlay_`, чтобы старые ярлыки/запуски продолжали открывать Micro mode.
- Основные режимы Ryazha остаются доступными через текущие аргументы `-mini`, `-micro`, `-fps_graph`, `-fps_counter` и `-game_resolutions`.
...
- ChanseyIsTheBest за тестирование меню разрешений aигр
