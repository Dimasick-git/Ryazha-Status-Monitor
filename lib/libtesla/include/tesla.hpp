...
/// Originally used the GNU statement-expression `({ ... })` extension, which
/// GCC 15 in the devkitpro container refuses to parse even with -std=gnu++23.
/// Rewritten as a do/while(0) macro: `return` exits the *enclosing* function
/// (whichever called the macro), so call-site semantics are unchanged.
#define TSL_R_TRY(resultExpr)                                                  \
    do {                                                                       \
        const auto _tsl_r_try_result = (resultExpr);                           \
        if (R_FAILED(_tsl_r_try_result)) {                                     \
            return _tsl_r_try_result;                                          \
        }                                                                      \
    } while (0)
