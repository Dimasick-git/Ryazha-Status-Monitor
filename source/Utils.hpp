...
// Global low-battery alert configuration (defaults)
inline bool lowBatteryAlertEnabled = false;
inline int  lowBatteryThresholdPercent = 15;

// Глобальный профиль темы (влияет на дефолтные цвета Mini/Micro до чтения config.ini)
inline std::string themeProfile = "dark"; // dark | light

// Глобальный профиль производительности: balanced | gaming | battery
inline std::string performanceProfile = "balanced";

// Глобальный алерт высокой температуры
inline bool highTempAlertEnabled = false;
inline int  highTempThresholdC = 63;

// Текущий язык интерфейса (ru | en)
inline std::string currentLanguage = "ru";
...
void ParseIniFile() {
    ...
    if (statusIt != configData.end()) {
        ...
        // theme_profile: "dark" | "light"
        auto themeProfileIt = statusSection.find("theme_profile");
        if (themeProfileIt != statusSection.end()) {
            key = themeProfileIt->second;
            convertToUpper(key);
            if (key == "LIGHT") themeProfile = "light"; else themeProfile = "dark";
        }

        // performance_profile: balanced | gaming | battery
        auto perfProfileIt = statusSection.find("performance_profile");
        if (perfProfileIt != statusSection.end()) {
            key = perfProfileIt->second;
            convertToUpper(key);
            if (key == "GAMING") performanceProfile = "gaming";
            else if (key == "BATTERY") performanceProfile = "battery";
            else performanceProfile = "balanced";
        }

        // High temperature alert settings
        auto highTempAlertIt = statusSection.find("high_temp_alert");
        if (highTempAlertIt != statusSection.end()) {
            key = highTempAlertIt->second;
            convertToUpper(key);
            highTempAlertEnabled = (key == "TRUE");
        }

        auto highTempThreshIt = statusSection.find("high_temp_threshold");
        if (highTempThreshIt != statusSection.end()) {
            long v = atol(highTempThreshIt->second.c_str());
            v = std::clamp(v, 40L, 90L);
            highTempThresholdC = (int)v;
        }

        // language: ru | en
        auto languageIt = statusSection.find("language");
        if (languageIt != statusSection.end()) {
            key = languageIt->second;
            convertToUpper(key);
            currentLanguage = (key == "EN") ? "en" : "ru";
        }

        // Low battery alert settings
        auto lowAlertIt = statusSection.find("low_battery_alert");
        if (lowAlertIt != statusSection.end()) {
            key = lowAlertIt->second;
            convertToUpper(key);
            lowBatteryAlertEnabled = (key == "TRUE");
        }

        auto lowThreshIt = statusSection.find("low_battery_threshold");
        if (lowThreshIt != statusSection.end()) {
            long v = atol(lowThreshIt->second.c_str());
            v = std::clamp(v, 1L, 100L);
            lowBatteryThresholdPercent = (int)v;
        }
    }
...
ALWAYS_INLINE void GetConfigSettings(MiniSettings* settings) {
    ...
    // Apply theme profile defaults before INI overrides
    if (themeProfile == "light") {
        convertStrToRGBA4444("#FFF8", &(settings->backgroundColor));
        convertStrToRGBA4444("#EEE8", &(settings->focusBackgroundColor));
        convertStrToRGBA4444("#333F", &(settings->separatorColor));
        convertStrToRGBA4444("#333F", &(settings->catColor));
        convertStrToRGBA4444("#000F", &(settings->textColor));
    }
...
ALWAYS_INLINE void GetConfigSettings(FullSettings* settings) {
    ...
    if (themeProfile == "light") {
        convertStrToRGBA4444("#333F", &(settings->separatorColor));
        convertStrToRGBA4444("#333F", &(settings->catColor1));
        convertStrToRGBA4444("#666F", &(settings->catColor2));
        convertStrToRGBA4444("#000F", &(settings->textColor));
    }
