...
constexpr const char* ryazhahandConfigIniPath = "sdmc:/config/ryazhahand/config.ini";
...
// Optional exponential moving-average smoothing. Every category owns distinct
// filter state so CPU data can never leak into GPU/RAM/temperature readings.
...
inline EmaSettings emaSettings{};
inline EmaFilterState emaFps{};
inline EmaFilterState emaCpuHz{}, emaGpuHz{}, emaRamHz{};
inline EmaFilterState emaRealCpuHz{}, emaRealGpuHz{}, emaRealRamHz{};
inline EmaFilterState emaGpuLoad{}, emaRamLoadAll{}, emaRamLoadCpu{}, emaRamBandwidth{}, emaRamBandwidthCpu{};
inline EmaFilterState emaSocTemp{}, emaPcbTemp{}, emaSkinTemp{}, emaCpuTemp{}, emaGpuTemp{}, emaRamTemp{};
inline EmaFilterState emaBatteryCurrent{}, emaBatteryVoltage{}, emaBatteryPower{};

inline float applyEma(EmaFilterState& filter, float sample, bool enabled) {
    if (!enabled) {
    ...
    filter.value = alpha * sample + (1.0f - alpha) * filter.value;
    return filter.value;
}

inline uint32_t applyEmaU32(EmaFilterState& filter, uint32_t sample, bool enabled, bool zeroIsValid = true) {
    if (!zeroIsValid && sample == 0) {
        filter.initialized = false;
        return sample;
    }
    const float smoothed = applyEma(filter, static_cast<float>(sample), enabled);
    return static_cast<uint32_t>(smoothed + 0.5f);
}
...
void BatteryChecker(void*) {
    ...
    do {
        ...
        batCurrentAvg = applyEma(emaBatteryCurrent, batCurrent, emaSettings.power);
        batVoltageAvg = applyEma(emaBatteryVoltage, batVoltage, emaSettings.power);
        batPowerAvg /= ArraySize * 1000;
        PowerConsumption = applyEma(emaBatteryPower, batPowerAvg, emaSettings.power);
...
void gpuLoadThread(void*) {
    ...
    if (!GPULoadPerFrame && R_SUCCEEDED(nvCheck) && R_FAILED(hocclkCheck)) do {
        ...
        if (R_SUCCEEDED(nvIoctl(fd, NVGPU_GPU_IOCTL_PMU_GET_GPU_LOAD, &temp)) && temp > 0) {
            ...
            const uint32_t average = std::accumulate(&gpu_load_array[0], &gpu_load_array[gpu_samples_average], 0) / gpu_samples_average;
            GPU_Load_u = applyEmaU32(emaGpuLoad, average, emaSettings.loads, false);
        }
...
void Misc(void*) {
    ...
    do {
        ...
        // Apply user-configured EMA only after raw values have been consumed by
        // their calculations (for example RAM load's raw EMC frequency divisor).
        CPU_Hz = applyEmaU32(emaCpuHz, CPU_Hz, emaSettings.frequencies);
        GPU_Hz = applyEmaU32(emaGpuHz, GPU_Hz, emaSettings.frequencies);
        RAM_Hz = applyEmaU32(emaRamHz, RAM_Hz, emaSettings.frequencies);
        realCPU_Hz = applyEmaU32(emaRealCpuHz, realCPU_Hz, emaSettings.frequencies);
        realGPU_Hz = applyEmaU32(emaRealGpuHz, realGPU_Hz, emaSettings.frequencies);
        realRAM_Hz = applyEmaU32(emaRealRamHz, realRAM_Hz, emaSettings.frequencies);
        SOC_temperatureF = applyEma(emaSocTemp, SOC_temperatureF, emaSettings.temperatures);
        PCB_temperatureF = applyEma(emaPcbTemp, PCB_temperatureF, emaSettings.temperatures);
        skin_temperaturemiliC = static_cast<int32_t>(applyEma(emaSkinTemp, static_cast<float>(skin_temperaturemiliC), emaSettings.temperatures));
        componentCPU_mC = applyEmaU32(emaCpuTemp, componentCPU_mC, emaSettings.temperatures);
        componentGPU_mC = applyEmaU32(emaGpuTemp, componentGPU_mC, emaSettings.temperatures);
        componentRAM_mC = applyEmaU32(emaRamTemp, componentRAM_mC, emaSettings.temperatures);
        ramLoad[SysClkRamLoad_All] = applyEmaU32(emaRamLoadAll, ramLoad[SysClkRamLoad_All], emaSettings.loads);
        ramLoad[SysClkRamLoad_Cpu] = applyEmaU32(emaRamLoadCpu, ramLoad[SysClkRamLoad_Cpu], emaSettings.loads);
        ramBW_MBs = applyEmaU32(emaRamBandwidth, ramBW_MBs, emaSettings.loads);
        ramBW_MBs_cpu = applyEmaU32(emaRamBandwidthCpu, ramBW_MBs_cpu, emaSettings.loads);
        ...
        GPU_Load_u = applyEmaU32(emaGpuLoad, GPU_Load_u, emaSettings.loads, false);
        ...
        if (GameRunning) {
            if (NxFps && SharedMemoryUsed) {
                ...
                FPSavg = applyEma(emaFps, FPSavg, emaSettings.fps);

                if (FPSavg > FPSmax) FPSmax = FPSavg;
                if (FPSavg < FPSmin) FPSmin = FPSavg;
            }
        } else {
            emaFps.initialized = false;
            FPSavg = 254;
            FPSmin = 254;
            FPSmax = 0;
        }
...
void ParseIniFile() {
    ...
    if (statusIt != configData.end()) {
        ...
        const auto readEma = [&](const char* name, bool& destination) {
            const auto option = statusSection.find(name);
            if (option == statusSection.end()) return;
            key = option->second;
            convertToUpper(key);
            destination = (key == "TRUE");
        };
        readEma("ema_fps", emaSettings.fps);
        readEma("ema_frequencies", emaSettings.frequencies);
        readEma("ema_loads", emaSettings.loads);
        readEma("ema_temperatures", emaSettings.temperatures);
        readEma("ema_power", emaSettings.power);
    }

    // Handle external combo - load each file once
    const struct { const char* path; const char* section; } externalConfigs[] = {
        {ryazhahandConfigIniPath, "ryazhahand"},
...
ALWAYS_INLINE void GetConfigSettings(FpsCounterSettings* settings) {
    ...
    it = section.find("show_refresh_hz");
    if (it != section.end()) {
        key = it->second;
        convertToUpper(key);
        settings->showRefreshHz = (key != "FALSE");
    }
...
ALWAYS_INLINE void GetConfigSettings(FullSettings* settings) {
    ...
    // Process boolean flags
    it = section.find("use_ryazha_theme");
    if (it != section.end()) {
        key = it->second;
        convertToUpper(key);
        settings->useRyazhaTheme = (key != "FALSE");
    }
