...
public:
    MicroOverlay() { 
        ...
        // Apply performance_profile overrides
        if (performanceProfile == "gaming") {
            TeslaFPS = (settings.refreshRate < 2) ? 2 : settings.refreshRate;
        } else if (performanceProfile == "battery") {
            TeslaFPS = 1;
            settings.useDynamicColors = false;
        ...
        while (start < show.length()) {
            ...
            if (len >= 3) {
                ...
                switch (key3) {
                    case 0x435055: // "CPU"
                        if (!(seen_flags & 1)) {
                            renderItems.push_back({0, (currentLanguage=="en"?"CPU":"ЦП"), CPU_compressed_c, CPU_volt_c, settings.realVolts});
                            seen_flags |= 1;
                        }
                        break;
                    case 0x475055: // "GPU"
                        if (!(seen_flags & 2)) {
                            renderItems.push_back({1, (currentLanguage=="en"?"GPU":"ГП"), GPU_Load_c, GPU_volt_c, settings.realVolts});
                            seen_flags |= 2;
                        }
                        break;
                    case 0x52414D: // "RAM"
                        if (!(seen_flags & 4)) {
                            renderItems.push_back({2, (currentLanguage=="en"?"RAM":"ПАМЯТЬ"), RAM_var_compressed_c, RAM_volt_c, settings.realVolts});
                            seen_flags |= 4;
                        }
                        break;
                    case 0x534F43: // "SOC"
                        if (!(seen_flags & 8)) {
                            renderItems.push_back({3, (currentLanguage=="en"?"SOC":"СОК"), soc_temperature_c, SOC_volt_c, settings.realVolts && settings.showSOCVoltage});
                            seen_flags |= 8;
                        }
                        break;
                    case 0x544D50: // "TMP"
                        if (!(seen_flags & 16)) {
                            renderItems.push_back({4, (currentLanguage=="en"?"TMP":"ТЕМП"), skin_temperature_c, SOC_volt_c, settings.realVolts && settings.showSOCVoltage});
                            seen_flags |= 16;
                        }
                        break;
                    case 0x524553: // "RES"
                        if (!(seen_flags & 128)) {
                            renderItems.push_back({7, (currentLanguage=="en"?"RES":"РАЗР"), RES_var_compressed_c, nullptr, false});
                            seen_flags |= 128;
                            resolutionShow = true;
                        }
                        break;
                    case 0x465053: // "FPS"
                        if (!(seen_flags & 32)) {
                            renderItems.push_back({5, (currentLanguage=="en"?"FPS":"КАДРЫ"), FPS_var_compressed_c, nullptr, false});
                            seen_flags |= 32;
                        }
                        break;
                    case 0x424154: // "BAT"
                        if (!(seen_flags & 64)) {
                            renderItems.push_back({6, (currentLanguage=="en"?"BAT":"БАТ"), Battery_c, nullptr, false});
                            seen_flags |= 64;
                        }
                        break;
                    case 0x524541: // "REA" (for READ)
                        if (len >= 4 && key[3] == 'D' && !(seen_flags & 512)) {
                            renderItems.push_back({9, (currentLanguage=="en"?"READ":"ЧТЕНИЕ"), READ_var_compressed_c, nullptr, false});
                            seen_flags |= 512;
                        }
                        break;
                    case 0x445443: // "DTC" 
                        if (!(seen_flags & 256) && settings.showDTC) {
                            renderItems.push_back({8, settings.useDTCSymbol ? "\uE007" : (currentLanguage=="en"?"DTC":"ДАТА"), DTC_c, nullptr, false});
                            seen_flags |= 256;
                        }
                        break;
                }
    ...
    virtual void update() override {
        ...
        // High-temp alert for Micro
        if (highTempAlertEnabled && (int)SOC_temperatureF >= highTempThresholdC) {
            static bool tempAlertActiveMicro = false;
            if (!tempAlertActiveMicro) {
                triggerRumbleDoubleClick.store(true, std::memory_order_release);
                triggerOnSound.store(true, std::memory_order_release);
                tempAlertActiveMicro = true;
            }
            strcat(soc_temperature_c, " !");
        }
