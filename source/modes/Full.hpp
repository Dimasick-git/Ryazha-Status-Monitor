...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            //Print strings
            ///CPU
            // Заголовки секций всегда отрисовываем
            renderer->drawString((currentLanguage=="en"?"CPU Usage":"Загрузка ЦП"), false, COMMON_MARGIN, 120, 20, (settings.catColor1));
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck)) {
                ...
                if (settings.showTargetFreqs) {
                    renderer->drawString((currentLanguage=="en"?"Target Frequency":"Целевая частота"), false, COMMON_MARGIN, height_offset, 15, (settings.catColor2));
                    renderer->drawString(CPU_Hz_c, false, COMMON_MARGIN + valueOffset, height_offset, 15, (settings.textColor));
                }
                if (realCPU_Hz && settings.showRealFreqs) {
                    renderer->drawString((currentLanguage=="en"?"Real Frequency":"Реальная частота"), false, COMMON_MARGIN, height_offset - 15, 15, (settings.catColor2));
                ...
                static auto core0Width = renderer->getTextDimensions((currentLanguage=="en"?"Core 0  ":"Ядро 0  "), false, 15).first;
                static auto core1Width = renderer->getTextDimensions((currentLanguage=="en"?"Core 1  ":"Ядро 1  "), false, 15).first;
                static auto core2Width = renderer->getTextDimensions((currentLanguage=="en"?"Core 2  ":"Ядро 2  "), false, 15).first;
                static auto core3Width = renderer->getTextDimensions((currentLanguage=="en"?"Core 3  ":"Ядро 3  "), false, 15).first;
                
                const uint32_t core_height = height_offset + 30;
                renderer->drawString((currentLanguage=="en"?"Core 0  ":"Ядро 0  "), false, COMMON_MARGIN, core_height, 15, (settings.catColor2));
                renderer->drawString(CPU_Core0_c, false, COMMON_MARGIN + core0Width, core_height, 15, (settings.textColor));
                
                renderer->drawString((currentLanguage=="en"?"Core 1  ":"Ядро 1  "), false, COMMON_MARGIN, core_height + 15, 15, (settings.catColor2));
                renderer->drawString(CPU_Core1_c, false, COMMON_MARGIN + core1Width, core_height + 15, 15, (settings.textColor));
                
                renderer->drawString((currentLanguage=="en"?"Core 2  ":"Ядро 2  "), false, COMMON_MARGIN, core_height + 30, 15, (settings.catColor2));
                renderer->drawString(CPU_Core2_c, false, COMMON_MARGIN + core2Width, core_height + 30, 15, (settings.textColor));
                
                renderer->drawString((currentLanguage=="en"?"Core 3  ":"Ядро 3  "), false, COMMON_MARGIN, core_height + 45, 15, (settings.catColor2));
                renderer->drawString(CPU_Core3_c, false, COMMON_MARGIN + core3Width, core_height + 45, 15, (settings.textColor));
            }
            
            ///GPU
            renderer->drawString((currentLanguage=="en"?"GPU Usage":"Загрузка ГП"), false, COMMON_MARGIN, 285-8, 20, (settings.catColor1));
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(nvCheck)) {
                ...
                if (R_SUCCEEDED(nvCheck)) {
                    renderer->drawString((currentLanguage=="en"?"Load":"Нагрузка"), false, COMMON_MARGIN, height_offset + 15, 15, (settings.catColor2));
            ...
            ///RAM
            renderer->drawString((currentLanguage=="en"?"RAM Usage":"Загрузка ПАМЯТИ"), false, COMMON_MARGIN, 375, 20, (settings.catColor1));
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(Hinted)) {
                ...
                if (R_SUCCEEDED(Hinted)) {
                    renderer->drawString((currentLanguage=="en"?"Total\nApplication\nApplet\nSystem\nSystem Unsafe":"Всего\nПриложения\nАпплеты\nСистема\nСистема (unsafe)"), false, COMMON_MARGIN, height_offset + 40, 15, (settings.catColor2));
            ...
            ///Thermal
            renderer->drawString((currentLanguage=="en"?"Board":"Плата"), false, 20, 550, 20, (settings.catColor1));
            if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck) || R_SUCCEEDED(pwmCheck)) {
                if (R_SUCCEEDED(i2cCheck)) {
                    renderer->drawString((currentLanguage=="en"?"Battery Power":"Пит. батареи"), false, COMMON_MARGIN, 575, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck)) {
                    ...
                    renderer->drawString((currentLanguage=="en"?"\nTemperatures":"\nТемпературы"), false, COMMON_MARGIN, 590, 15, (settings.catColor2));
                ...
                if (R_SUCCEEDED(pwmCheck)) {
                    renderer->drawString((currentLanguage=="en"?"Fan":"Вентилятор"), false, COMMON_MARGIN, 635, 15, (settings.catColor2));
            ...
            if (GameRunning) {
                ...
                if (settings.showFPS || settings.showRES || settings.showRDSD) {
                    renderer->drawString((currentLanguage=="en"?"Game":"Игра"), false, COMMON_MARGIN + width_offset, 185+12, 20, (settings.catColor1));
                }
                ...
                if ((settings.showRES == true) && NxFps && SharedMemoryUsed && (NxFps -> API >= 1)) {
                    static auto resLabelWidth = renderer->getTextDimensions((currentLanguage=="en"?"Resolutions  ":"Разрешения  "), false, 15).first;
                    renderer->drawString((currentLanguage=="en"?"Resolutions  ":"Разрешения  "), false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
                ...
                if (settings.showRDSD == true) {
                    static auto readLabelWidth = renderer->getTextDimensions((currentLanguage=="en"?"Read Speed  ":"Скорость чтения  "), false, 15).first;
                    renderer->drawString((currentLanguage=="en"?"Read Speed  ":"Скорость чтения  "), false, COMMON_MARGIN + width_offset, height, 15, (settings.catColor2));
                    renderer->drawString(readSpeed_c, false, COMMON_MARGIN + width_offset + readLabelWidth, height, 15, (settings.textColor));
                }
            }
            
            static const auto pressWidth = renderer->getTextDimensions((currentLanguage=="en"?"Press ":"Нажмите "), false, 23).first;
            ...
            // Draw "Нажмите "
            renderer->drawString((currentLanguage=="en"?"Press ":"Нажмите "), false, baseX, baseY, fontSize, (tsl::bottomTextColor));
            ...
            // Draw " для выхода"
            renderer->drawString((currentLanguage=="en"?" to Exit":" для выхода"), false, baseX + pressWidth + keyComboWidth, baseY, fontSize, (tsl::bottomTextColor));
        });
        
        auto rootFrame = new tsl::elm::HeaderOverlayFrame("Ryazha-Status Монитор", APP_VERSION);
