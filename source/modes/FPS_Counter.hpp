...
private:
    char FPSavg_c[8];
    char FPSHz_c[32] = "-- FPS / 60 Гц";
    uint8_t displayRefreshHz = 60;
...
public:
    com_FPS() { 
        ...
        // Query the actual display refresh rate once at launch. The shared-memory
        // value is used for live updates below when it is available.
        if (R_SUCCEEDED(SaltySD_Connect())) {
            if (R_FAILED(SaltySD_GetDisplayRefreshRate(&displayRefreshHz)) || displayRefreshHz == 0)
                displayRefreshHz = 60;
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto* Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            const char* displayText = (FPSavg != 254.0) ? FPSHz_c : (settings.showRefreshHz ? "-- FPS / -- Гц" : "--");
            const auto [textWidth, textHeight] = renderer->getTextDimensions(
                displayText, false, fontsize
            );
    ...
    virtual void update() override {
        ...
        if (shouldUpdateData) {
            ...
            // SaltyNX publishes the current display refresh rate in the same
            // shared block as FPS. Fall back to the launch-time display query.
            if (NxFps && SharedMemoryUsed && NxFps->currentRefreshRate > 0)
                displayRefreshHz = NxFps->currentRefreshRate;
            if (displayRefreshHz == 0)
                displayRefreshHz = 60;

            if (settings.showRefreshHz) {
                snprintf(FPSHz_c, sizeof FPSHz_c, "%s FPS / %u Гц", FPSavg_c, displayRefreshHz);
            } else {
                snprintf(FPSHz_c, sizeof FPSHz_c, "%s", FPSavg_c);
            }
        }
    ...
    virtual bool handleInput(u64 keysDown, u64 keysHeld, const HidTouchState &touchPos, HidAnalogStickState joyStickPosLeft, HidAnalogStickState joyStickPosRight) override {
        ...
        if (pinchDetected) {
            ...
            if (!pinchActive || pinchStartDistance < 1.0f) {
                ...
                settings.handheldFontSize = static_cast<size_t>(std::clamp<int>(std::lround(pinchStartHandheld * scale), 8, 72));
                settings.dockedFontSize = static_cast<size_t>(std::clamp<int>(std::lround(pinchStartDocked * scale), 8, 96));
                settings.docked1080pFontSize = static_cast<size_t>(std::clamp<int>(std::lround(pinchStart1080 * scale), 10, 128));
            ...
            iniData["fps-counter"]["handheld_font_size"] = std::to_string(settings.handheldFontSize);
            iniData["fps-counter"]["docked_font_size"] = std::to_string(settings.dockedFontSize);
            iniData["fps-counter"]["docked_1080p_font_size"] = std::to_string(settings.docked1080pFontSize);
