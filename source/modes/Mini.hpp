...
public:
        ...
        auto* Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            if (needsRecalc) {
                ...
                for (const auto& key : showKeys) {
                    ...
                    if (key == "CPU" && !(flags & 1)) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "CPU" : "ЦП");
                        flags |= 1;
                    } else if (key == "GPU" && !(flags & 2)) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "GPU" : "ГП");
                        flags |= 2;
                    } else if (key == "RAM" && !(flags & 4)) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "RAM" : "ПАМЯТЬ");
                        flags |= 4;
                    } else if (key == "SOC" && !(flags & 8)) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "SOC" : "СОК");
                        flags |= 8;
                    } else if (key == "TMP" && !(flags & 16)) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "TMP" : "ТЕМП");
                        flags |= 16;
                    } else if ((key == "BAT" || key == "DRAW") && !(flags & 32)) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "BAT" : "БАТ");
                        flags |= 32;
                    } else if (key == "FPS" && !(flags & 64) && GameRunning) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "FPS" : "КАДРЫ");
                        flags |= 64;
                    } else if (key == "RES" && !(flags & 128) && GameRunning) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "RES" : "РАЗР");
                        flags |= 128;
                        resolutionShow = true;
                    } else if (key == "READ" && !(flags & 512) && GameRunning) {
                        shouldAdd = true;
                        labelText = (currentLanguage == "en" ? "READ" : "ЧТЕНИЕ");
                        flags |= 512;
                    } else if (key == "DTC" && !(flags & 256) && settings.showDTC) {
                        shouldAdd = true;
                        labelText = settings.useDTCSymbol ? "\uE007" : (currentLanguage == "en" ? "DTC" : "ДАТА");
                        flags |= 256;
                    }
            ...
            const uint32_t margin = (fontsize * (currentLanguage == "ru" ? 5 : 4));
    ...
    virtual void update() override {
        ...
        // High-temp alert marker for SOC line
        ...
        // Low-battery alert: haptic + visual marker
        static bool lowAlertActive = false;
        const float chargePct = (float)_batteryChargeInfoFields.RawBatteryCharge / 1000.0f;
        const bool isDischarging = (drawW > 0.05f);
        if (lowBatteryAlertEnabled) {
            if (isDischarging && chargePct <= (float)lowBatteryThresholdPercent) {
                if (!lowAlertActive) {
                    triggerRumbleDoubleClick.store(true, std::memory_order_release);
                    triggerOnSound.store(true, std::memory_order_release);
                    lowAlertActive = true;
                }
                strcat(SoCPCB_temperature_c, " !");
            } else if (lowAlertActive && chargePct > (float)lowBatteryThresholdPercent + 1) {
