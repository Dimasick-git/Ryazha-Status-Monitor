...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto* langItem = new tsl::elm::ListItem("Language");
        langItem->setValue(currentLanguage == "en" ? "English" : "Русский");
        langItem->setClickListener([this, langItem](uint64_t keys){
            if (keys & KEY_A) {
                const std::string next = (currentLanguage == "en") ? "ru" : "en";
                ult::setIniFileValue(configIniPath, "status-monitor", "language", next);
        ...
        // Reset colors to defaults for current mode
        auto* resetColors = new tsl::elm::ListItem("Reset Colors");
        resetColors->setValue("!");
        resetColors->setClickListener([this](uint64_t keys){
            if (keys & KEY_A) {
                std::string section;
                if (isMiniMode) section = "mini";
                else if (isMicroMode) section = "micro";
                ...
                // Apply theme defaults for section
                auto apply = [&](const std::string& key, const std::string& val){ ult::setIniFileValue(configIniPath, section, key, val); };

                const bool light = (themeProfile == "light");
                if (isFullMode) {
                    apply("separator_color", light ? "#333F" : "#2DFF");
                    apply("cat_color_1",   light ? "#333F" : "#8FFF");
                    apply("cat_color_2",   light ? "#666F" : "#8CFF");
                    apply("text_color",     light ? "#000F" : "#FFFF");
                } else if (isMiniMode) {
                    apply("background_color",       light ? "#FFF8" : "#0009");
                    apply("focus_background_color", light ? "#EEE8" : "#000F");
                    apply("separator_color",        light ? "#333F" : "#2DFF");
                    apply("cat_color",              light ? "#333F" : "#2DFF");
                    apply("text_color",             light ? "#000F" : "#FFFF");
                } else if (isMicroMode) {
                    apply("background_color",       light ? "#FFF8" : "#0009");
                    apply("separator_color",        light ? "#333F" : "#2DFF");
                    ...
                    apply("background_color",       light ? "#FFF8" : "#1117");
                    apply("cat_color",              light ? "#000F" : "#FFFF");
                    apply("text_color",             light ? "#000F" : "#FFFF");
                } else if (isFPSCounterMode) {
                    apply("background_color",       light ? "#FFF8" : "#1117");
                    apply("text_color",             light ? "#000F" : "#FFFF");
                } else if (isFPSGraphMode) {
                    apply("background_color",       light ? "#FFF8" : "#1117");
                    apply("fps_counter_color",      light ? "#8888" : "#4444");
                    apply("border_color",           light ? "#F00F" : "#F77F");
                    apply("dashed_line_color",      light ? "#8888" : "#8888");
                    apply("max_fps_text_color",     light ? "#FFFF" : "#FFFF");
                    apply("min_fps_text_color",     light ? "#FFFF" : "#FFFF");
                    apply("main_line_color",        light ? "#FFFF" : "#FFFF");
                    apply("rounded_line_color",     light ? "#F0FF" : "#F0FF");
                    apply("perfect_line_color",     light ? "#0C0F" : "#0C0F");
                }
                triggerRumbleDoubleClick.store(true, std::memory_order_release);
                triggerOnSound.store(true, std::memory_order_release);
                return true;
